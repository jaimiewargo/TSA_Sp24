...
plot_grid(autoplot(model1$residuals),
          autoplot(Acf(model1$residuals, lag.max = 40, plot = F)),
          autoplot(Pacf(model1$residuals, lag.max = 40, plot=F)))
```
...
plot_grid(autoplot(model2$residuals),
          autoplot(Acf(model2$residuals, lag.max = 40, plot = F)),
          autoplot(Pacf(model2$residuals, lag.max = 40, plot=F)))

autoplot(ts_electricity_price[,'Residential'], series='Original')+
  autolayer(model1$fitted, series='Model 1')+
  autolayer(model2$fitted, series='Model 2')
```
