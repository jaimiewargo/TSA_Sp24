...
raw_inflow_data <- read.table(file=here('Data', 'inflowtimeseries.txt'),
                              header=FALSE,skip=0)
...
autoplot(inflow_random)+
  autolayer(tsclean(inflow_random), color="steelblue3")

#tsclean() removes outliers from additive series
```
...
ggplot(df_inflow_random, aes(x=date, y=inflow)) +
            geom_line() +
            geom_abline(slope=0,intercept=3*sd_inflow,color="red3") +
            geom_abline(slope=0,intercept=-3*sd_inflow,color="red3")
