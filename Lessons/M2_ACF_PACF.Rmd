...
ggplot(data=inflow_data, aes(x=my_date, y=HP1))+
  geom_line(color='darkseagreen')

#adding two time series to the same plot
ggplot(data=inflow_data, aes(x=my_date))+
  geom_line(aes(y=HP1), color='darkseagreen')+
  geom_line(aes(y=HP2), color='steelblue3')+
  labs(y=expression(paste("Inflow (", m^{3}, '/s)')))
```
